	...
	while (client.isClientRunning) {
		...
		if (!update.empty()) {
			...
			case 5: //update position 4 playerid position rotation
				...
				playerPosition[id].y = posx;
				playerPosition[id].z = posx;
...
void mainThread()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		/*cout << "position" << playerPosition.size() << endl;
		cout << "rotation" <<playerRotation.size() << endl;
		cout << "ID" <<playerId.size() << endl;*/
