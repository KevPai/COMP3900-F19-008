...
int checkCollision(Model player, glm::vec3& position, Cube toTest, float scale) {	
	...
	if ( boundary(position.x - toTest.getX() , player.getDimX()[0] * scale + 0.5f)) {
		...
		else if (between( (position.z + player.getDimZ()[1] * scale) - toTest.getZ() , back, front)) {
			return -3;
		}
	}
		...
		else if (between((position.x + player.getDimX()[1] * scale) - toTest.getX(), back, front)) {
			return -1;
		}
	}
	return 0;
